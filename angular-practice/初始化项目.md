...
Angular是单页面应用。可以这么理解，所以的操作和显示都在index.html。

<div style="text-align:center"><img src="pictures/angular-index.png" width="200px"></div>

通过显示或隐藏特定组件的显示部分来改变用户能看到的内容。画面之间的跳转，实质上是HTML页面内标签的替换。

index.html文件内的标签\<app-root>指向app.component.html。

所以app.component.html是用户看到的第一个画面，并且其一直显示。
...
当然也有一些架构设计者会选择在app.component.html显示网站的header和sidebar。仅在content内进行内容替换。
